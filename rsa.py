...
role = st.radio("", ["受信者", "一人で行う"], horizontal=True)
st.markdown("---")

# ========== 受信者モード ==========
if role == "受信者":
    st.header("1. 鍵生成（受信者）")
    st.caption("p, q を選ぶと e が自動的に計算されます。")
    ...
    phi = (p - 1) * (q - 1)
    if p == q:
        e_auto, e_display = None, "p と q は異なる素数を選んでください。"
    else:
        try:
            e_auto = auto_select_e(phi, p, q)
            e_display = str(e_auto)
        except ValueError:
            e_auto, e_display = None, "条件を満たす e がありません。"

    with c3:
        st.markdown("**公開鍵 e（自動計算）**")
        st.code(e_display)
    ...
    if st.button("鍵生成", key='recv_gen'):
        if p == q:
            ...
            st.error("e が求まりません。p, q を変更してください。")
        else:
            n = p * q
    ...
    if st.session_state.get("done_recv", False):
        ...
        with d1:
            n_in = st.text_input("公開鍵 n", "", placeholder="n を貼り付け", key='dec_n')
        with d2:
            d_in = st.text_input("秘密鍵 d", "", placeholder="d を貼り付け", key='dec_d')
        with d3:
            c_in = st.text_area("暗号文 (Base64)", "", placeholder="暗号文を貼り付け", key='dec_c')
...
# ========== 一人で行うモード ==========
    ...
    phi1 = (p - 1) * (q - 1)
    if p == q:
        e_auto1, e_display1 = None, "p と q は異なる素数を選んでください。"
    else:
        try:
            e_auto1 = auto_select_e(phi1, p, q)
            e_display1 = str(e_auto1)
        except ValueError:
            e_auto1, e_display1 = None, "条件を満たす e がありません。"
    ...
    if st.session_state.get("done_solo", False):
        ...
        with oc1:
            n_enc = st.text_input("公開鍵 n", "", placeholder="n を貼り付け", key='solo_enc_n')
        with oc2:
            e_enc = st.text_input("公開鍵 e", "", placeholder="e を貼り付け", key='solo_enc_e')
        with oc3:
        ...
        with dc1:
            n_dec = st.text_input("公開鍵 n", "", placeholder="n を貼り付け", key='solo_dec_n')
        with dc2:
            d_dec = st.text_input("秘密鍵 d", "", placeholder="d を貼り付け", key='solo_dec_d')
        with dc3:
            ciph = st.text_area("暗号文 (Base64)", "", placeholder="暗号文を貼り付け", key='solo_dec_c')
